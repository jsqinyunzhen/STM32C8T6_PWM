...
{ 	
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4;    //PA0 作为模拟通道输入引脚    
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;		//模拟输入引脚
	GPIO_Init(GPIOA, &GPIO_InitStructure);

  /*GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4;    //PA2 作为模拟通道输入引脚    
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;		//模拟输入引脚
	GPIO_Init(GPIOA, &GPIO_InitStructure);		*/
...
//得到ADC采样光敏传感器的值
//取10次,然后平均
u16 Get_SENSOR10(void)
	{
	u16 temp_val=0;
	u8 t;
	for(t=0;t<10;t++)
		{
		temp_val+=Get_Adc3(ADC_Channel_6);	  //TampSensor
		delay_ms(5);
		}
	return temp_val/10;
	}*/
